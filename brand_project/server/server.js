...
app.use('/', express.static('./html')); // запросы в корень нашего сайт отдают содержимое public
...
app.delete('/api/cart/:id', (req, res) => {
  fs.readFile('./server/db/userCart.json', 'utf-8', (err, data) => {
    if (err) {
      ...
      const cart = JSON.parse(data);
      // ищем товар по id
      const findItem = cart.contents.findIndex(el => el.id_product === +req.params.id);
     cart.contents.splice(findItem,1);
      // пишем обратно
      fs.writeFile('./server/db/userCart.json', JSON.stringify(cart), (err) => {
        if (err) {
          res.send('{"result": 0}');
