class ComponentList {
    ...
    constructor() {
        this.topings = [
            {ruName: 'Горчица', enName: 'toping-1', cost: 15, cal: 0},
    ...
    _createDOMForBurger() {
        ...
        const sizeSelectHTML = this.sizes.reduce((content, item) => {
            return `${content} <option value="${item.enName}">${item.ruName}</option>`;
        }, '');
        ...
        const fillSelectHTML = this.filles.reduce((content, item) => {
            return `${content} <option value="${item.enName}">${item.ruName}</option>`;
        }, '');
        ...
        const topingSelectHTML = this.topings.reduce((content, item) => {
